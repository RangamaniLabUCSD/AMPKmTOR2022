...
%% panel 2
subplot(3,2,4)
currentheatmap2 = heatmap(freq_vary,parscan,mTORC1_AUC);
ylabel('\bf{V_{IR} Multiplier}')
xlabel('\bf{Stimulus Frequency [Hz]}')
...
%% panel 3
subplot(3,2,6)
currentheatmap3 = heatmap(freq_vary,parscan,mTORC2_AUC);
ylabel('\bf{V_{IR} Multiplier}')
xlabel('\bf{Stimulus Frequency [Hz]}')
...
subplot(3,2,1)
hold on
plot(t,y(:,pAMPK),'Color',colorvector(VIR_ind,:),'LineStyle',char(markervector(freq_ind)))
xlabel('Time [s]')
...
title('AMPK')
hold on
plot(t,y(:,pmTORC1),'Color',colorvector(VIR_ind,:),'LineStyle',char(markervector(freq_ind)))
xlabel('Time [s]')
ylabel('Concentration [\muM]')
plotformat
xlim([-50,100])
...
subplot(3,2,5)
hold on
plot(t,y(:,pmTORC2),'Color',colorvector(VIR_ind,:),'LineStyle',char(markervector(freq_ind)))
xlabel('Time [s]')
ylabel('Concentration [\muM]')
plotformat
xlim([-50,100])
